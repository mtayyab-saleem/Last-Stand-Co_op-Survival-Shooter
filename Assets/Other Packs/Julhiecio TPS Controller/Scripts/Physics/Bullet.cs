...
namespace JUTPS.WeaponSystem
{
    ...
    public class Bullet : MonoBehaviour
    {
        ...
        void OnCollisionEnter(Collision col)
        {
            ...
            {
                // Friendly fire check must happen before any damage, force, hit marker or particle.
                // If the bullet hits a teammate, ignore all of that teammate's colliders
                // so the bullet passes through without any hit reaction.
                if (FriendlyFireUtility.IsSameTeam(Owner, col.gameObject))
                {
                    LSPlayer teammate = col.gameObject.GetComponentInParent<LSPlayer>();
                    if (teammate != null)
                    {
                        Ignore(teammate.GetComponentsInChildren<Collider>());
                    }
