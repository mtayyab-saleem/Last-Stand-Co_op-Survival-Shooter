...
namespace JUTPS
{
    ...
    public class JUCharacterController : JUCharacterBrain
    {
        ...
        protected virtual void Movement()
        {
            ...
            // ---------------------------------------------------------
            // >>> NEW FIX START: SLOPE STABILIZER (Anti-Ramp Effect)
            // ---------------------------------------------------------
            // Agar player zameen par hai, jump nahi kar raha, aur gaari mein nahi hai
            if (IsGrounded && !IsJumping && !IsDriving && !IsRagdolled)
            {
                // Is value (20f) ko barha ya kam kar k adjust karein agar zaroorat paray
                float downForce = 20f;

                // Rigidbody par extra neechay ki force lagana
